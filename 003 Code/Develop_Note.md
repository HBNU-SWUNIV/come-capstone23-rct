...
- 대체로 긍정적인 반응, 설계방법(프로토타입 제작 후 테스트하며 요구사항 변경)과 요구사항 정립 및 이를 통해서 개발한 과정에서 칭찬 받음
  ...
  - 이재흥 교수님 : 
    ...
    - EdgeComputing을 이용해 Camer Data 직통송신.
  - 진영택 교수님 : 
    - 질문 X
...
- Unity
  1. Oculus 기기로만의 작동
  2. 다중접속을 위한 UI 설계 및 제작
    1) 로그인 (패스워드 기반)
      - 회원가입 버튼 필요
    2) IP확인(=로딩) 
    3) 기기 (선택, 등록) 
      - 선택 : 등록되어져있는 기기 목록(연결까지되어있는것
      - 등록 : 기기의 IP등록을 통한 기기 필수 요건 등록(애매모호) - 좀더고민

  3. 기계학습을 이용한 감도 UI조절에 따른 기기의 동작 범위 설정

  4. 조종 UI목록
    1) 조종 감도(의자 스틱, 퀘스트2 버튼(조절 x, 기능 설정만))
    2) 카메라 위치 변경(설정 버튼 따로 만들기, 및 카메라 시야각의 이동)


- Server, Webrtc
  1. 영상처리
    1) 고해상도와 실시간성을 유지하기 위한 영상처리(블록 단위로 나누어 전송)
   
  2. 다중접속을 위한 서버 구축
    1) 다중접속시 오큘러스 기기와 카메라 연동을 위한 서버 구축(사용자 ID 정의)
   

# 2023.07.05

  1. 감도(의자를 통한 조종, 의자 위치 조절)
  2. 영상처리(EdgeComputing)
  3. UI(로그인X => 단순한 패스워드)
  4. 기기 동시 조종(미로봇 2대 동시조종)
  5. 다중 접속 X
  6. 카메라 하나 그대로 대신 360도 카메라 범위 조절 및 범위 밖 물체는 포인터를 이용한 사물구현 유니티에서
